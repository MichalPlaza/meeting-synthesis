...
async def handle_meeting_upload(db: AsyncIOMotorDatabase, meeting_data: MeetingCreate, audio_file: UploadFile):
    ...
    with tempfile.NamedTemporaryFile(delete=True, suffix=f".{suffix}") as tmp:
        ...
        updated_meeting = await crud_meetings.update_meeting_transcription(db, str(meeting.id), text)
