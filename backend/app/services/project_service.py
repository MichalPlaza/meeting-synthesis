...
async def get_projects_filtered(
    db: AsyncIOMotorDatabase, q: str | None, sort_by: str
) -> list[Project]:
    return await crud_projects.get_projects_filtered(db, q=q, sort_by=sort_by)
...
async def get_projects_with_member(
    ...
    return await crud_projects.get_projects_with_member(db, member_id)