...
async def create_project(project_in: ProjectCreate, db: AsyncIOMotorDatabase = Depends(get_database)):
...
async def projects_by_owner(owner_id: str, db: AsyncIOMotorDatabase = Depends(get_database)):
...
async def projects_by_member(member_id: str, db: AsyncIOMotorDatabase = Depends(get_database)):
    projects = await project_service.get_projects_with_member(db, member_id)
    return projects


@router.put("/{project_id}", response_model=ProjectResponse)
async def update_project(project_id: str, project_in: ProjectUpdate, db: AsyncIOMotorDatabase = Depends(get_database)):
